...
#+RESULTS:
#+begin_example
sec   rsa4096 2023-09-26 [SCEA]
      AD66C2AB06C68FAF37C99BEA6F6A844FE5CE7F91
uid           [ultimate] k8s.co.sharing.io (flux secrets)
ssb   rsa4096 2023-09-26 [SEA]
...
cd ~/infra/clusters/co.sharing.io
...
export KEY_NAME="k8s.co.sharing.io"
export KEY_FP=$(gpg --list-secret-keys "${KEY_NAME}" | grep SCEA -A1 | tail -1 | awk '{print $1}')
cd ~/infr/acluster/sco.sharing.io
cat <<EOF >> ./.sops.yaml
creation_rules:
  - path_regex: .*.yaml
...
* cert manager
** list zone
#+begin_src shell :sync :dir "/ssh:root@container.sharing.io:/"
kubectl -n powerdns exec svc/auth-dns -- \
    pdnsutil list-zone co.sharing.io
#+end_src
...
# tramp-dir: "/ssh:root@co.sharing.io:/"
